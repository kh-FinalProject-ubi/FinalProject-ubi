...
import "../styles/TermsAndPrivacyModal.css"; // 필요한 경우 스타일 작성

const TermsAndPrivacyModal = ({ open, onClose, view }) => {
  const 이용약관_본문 = `제1조 (목적)
...
이 약관은 대한민국 법률에 따라 해석되며, 회사와 이용자 간 분쟁에 대하여는 회사의 본사 소재지를 관할하는 법원을 제1심 관할법원으로 합니다.`;
  const 개인정보처리방침_본문 = `우비(UBI) 개인정보 처리방침

우비(UBI)(이하 “회사”)는 이용자의 개인정보를 소중하게 생각하며, 「개인정보 보호법」을 비롯한 관련 법령을 준수하고 있습니다. 본 개인정보 처리방침은 회사가 어떤 정보를 수집하고, 어떻게 이용하며, 이용자의 권리를 어떻게 보호하는지를 설명합니다.
...
회사는 개인정보 수집 및 이용 목적이 달성된 후에는 해당 정보를 지체 없이 파기합니다. 다만, 관련 법령에 따라 일정 기간 보관이 필요한 경우에는 그에 따릅니다.
항목보관 기간보관 근거회원 정보(이메일 등)탈퇴 후 7일 이내 파기내부 정책문의 및 고객지원 기록3년소비자기본법접속 로그, IP 등3개월통신비밀보호법
제4조 (개인정보의 제3자 제공)
...
수탁업체위탁업무 내용보유 및 이용 기간Amazon Web Services인프라 및 데이터 보관계약 종료 시까지Google Analytics이용자 행동 분석익명 통계 기반, 개별 식별 불가
...
회사는 원칙적으로 만 14세 미만 아동의 개인정보를 수집하지 않습니다.  만약 수집이 필요한 경우에는 법정대리인의 동의를 필수로 받고, 관련 법령에 따라 안전하게 보호합니다.
    ...
    <div className="modal-overlay">
      <div className="modal-content">
        ...
        <div className="modal-scrollbox">
          <p>{view === "terms" ? 이용약관_본문 : 개인정보처리방침_본문}</p>
        </div>
