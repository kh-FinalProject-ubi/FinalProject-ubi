...
export default function WelfareFacilityModal({ onSelect }) {
  ...
  const filteredFacilities = combinedFacilities.filter((f) => {
    ...
    const type =
      f["상세유형"] || f["시설종류명"] || f["SVC_TYPE"] || f["category"] || ""; // 시설유형
    ...
    console.log(userFull); // 서울특별시 강남구 표시
    console.log(regionFull); // 서울특별시 종로구 표시
    console.log(facilityAddr);

    return (
      matchesKeyword && matchesServiceType && matchesCategory && isSameRegion
    );
  });
    ...
    <div className="facility-search-container">
      ...
      {filteredFacilities.map((facility, idx) => {
        const name =
          ...
          "시설";
        const serviceId = facility["시설코드"] || "UNKNOWN_ID";
