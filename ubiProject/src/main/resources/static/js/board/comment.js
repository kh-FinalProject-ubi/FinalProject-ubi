...
// 구분 하여 자원의 상태(State)를 주고 받는 것(Transfer)
...
const selectCommentList = () => {
    ...
    .then(resp => resp.json()) // commentList를 가져와야 하니 json으로 / text는 단일값 가져오는 구문
    .then(commentList => {
        // console.log(commentList);
        
        // 화면에 존재하는 기존 댓글 목록 삭제 후
        // 조회한 commentList를 이용해서 새로운 댓글 목록 출력
...
const insertChildComment = (parentCommentNo, btn) => {
  ...
  const textara = btn.parentElement.previousElementSibling;

  // 유효성 검사
  if(textara.value.trim().length == 0) {
  ...
  // ajax를 이용해 전달할 데이터
  const data = {
    "commentContent" : textara.value,  // 작성한 글
