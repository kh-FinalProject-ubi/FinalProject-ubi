...
public class EmailConfig {
	/*
	 * SMTP (Simple Main Transfer Protocol) 이메일을 전송하는 데 사용되는 표준 통신 프로토콜 동작방식 1.
	 * Spring boot 어플리케이션에서 내가 이메일 보냄 2. Google SMTP 서버로 전송 (Google 서버가 중간 배달자 역할)
	 * 3. Google SMTP 서버가 이메일 주소에 맞는 "수신자"에게 이메일 서버 (GMAIL, NAVER 회사 이메일 서버)을 통해 발송
	 * 4. 최종 수신자 이메일을 받음
	 * 
	...
	public JavaMailSender javaMailSender() {
		 ...
		 * 
		 * JavaMailSender : 이메일을 보내는 데 사용되는 인터페이스로 JavaMailSenderImpl 클래스를 통해 구현됨. SMTP
		 * 서버를 이용하여 이메일을 보내기 위한 구성을 제공
		 *
		 */
